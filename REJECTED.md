...
rejected, Day 4 would close at −155.00, so no reading of the stream gives exactly one fee. A test asserts the three.
...
accepted: a customer can be debited for a payment the ledger never approved. A test asserts that E6 debits 180.00.
...
dating the refunds into days already closed, and the ledger does neither. A test asserts each of these figures.
...
so E10 posts 3.333, 3.333, and 3.334, which sum to exactly 10.000. A test asserts the three amounts and their sum.
...
rather than accepted as vacuously true. A test asserts that each capitalization equals the sum of its interest events.
