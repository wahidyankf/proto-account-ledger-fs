...
- **2026-09-25 19:03, Phase 0.** The first evidence file failed `check:hygiene`: the directory-map gate requires every
  ...
  and neither the plans convention nor the template names it. Triage 2026-09-25 21:18: discarded. The directory-map gate
...
- **2026-09-25 19:35, Phase 1.** The scratch width checker walked only directories, so a Markdown file named on its
  ...
  proves nothing, so each scratch check should fail loudly on an argument it cannot use. Triage 2026-09-25 21:18:
...
- **2026-09-25 20:24, Phase 4.** The first run of the seven ban mutations counted no failures: it grepped ruff's output
  ...
  failure has been seen to match; each such grep should be tried once against a failure first. Triage 2026-09-25 21:18:
...
- **2026-09-25 21:13, Phase 8.** The Phase 8 record line and its WORKLOG entry were left out of 26f2e12: the script that
  ...
  anchor into Prettier-formatted text should be read from the file just before it is used. Triage 2026-09-25 21:18:
