...
- [Domain Model](001-domain-model.md) — money, identifiers, accounts, events, the log, balances, and the authorization
  machine.
- [Replay and End of Day](002-replay-and-end-of-day.md) — the driver, processing, the three end-of-day steps, and the
  day report.
- [Input, Output, and the Command Line](003-input-output-and-cli.md) — the CSV stream, parse faults, rendering rules,
  and the exit contract.
- [Testing Strategy](004-testing-strategy.md) — the three layers, every test file, and the one strict expected failure.
- [Decision Records](005-decision-records.md) — every material choice, its alternatives, and when to revisit it.
- [Specification and Rule Changes](006-specification-and-rule-changes.md) — the architecture delta and the rule changes,
  file by file.
- [File Impact](007-file-impact.md) — every path the plan touches, as one annotated tree.
