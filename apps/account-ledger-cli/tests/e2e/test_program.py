...
def test_the_brief_replay_prints_output_target() -> None:
    ...
    completed = program(str(CHALLENGE_STREAM))

    assert completed.stdout.split("\n") == expected_output().split("\n")  # a failure names the first line that differs
    assert (completed.stderr, completed.returncode) == ("", 0)
...
def test_a_missing_stream_file_exits_2() -> None:
    ...
    completed = program("streams/missing.csv")

    assert (completed.stdout, completed.stderr, completed.returncode) == (
        "",
...
def test_a_closed_pipe_exits_141_quietly() -> None:
    ...
    assert (completed.stderr, completed.returncode) == ("", 141)
