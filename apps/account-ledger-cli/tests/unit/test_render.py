...
def test_a_day_opens_with_its_banner_and_its_blocks() -> None:
    ...
    result = replay_stream((), CHALLENGE)
...
def test_an_empty_block_prints_none() -> None:
    ...
    lines = render_reports((replay_stream((), CHALLENGE).find_report(Day(0)),)).split("\n")
...
def test_amounts_print_in_their_currencys_format() -> None:
    ...
    day_1 = render_reports((replay_stream(opening, CHALLENGE).find_report(Day(1)),)).split("\n")
    day_5 = render_reports((replay_stream(build_brief_stream(), CHALLENGE).find_report(Day(5)),)).split("\n")
...
def test_each_row_prints_its_type_and_detail() -> None:
    ...
    days = [
        ...
        for day in replay_stream(stream, CHALLENGE).reports[1:5]
    ]
...
def test_instalment_counts_print_as_words() -> None:
    ...
    def list_details(count: int) -> list[str]:
        ...
        return [
            ...
            for _, detail in _list_types_and_details(
                _select_blocks(render_reports((replay_stream(stream, CHALLENGE).find_report(Day(1)),)))
            )
        ]
    ...
    assert [list_details(count)[0] for count in (2, 10, 11)] == [
...
def test_authorization_states_print_as_output_target_shows() -> None:
    ...
    stream = (
        ...
        make_authorization("E3", 1, "Auth-B", "90.00"),
        make_settlement("E4", 2, "Auth-A", "45.00"),
    )

    def list_cells(day: int) -> list[str]:
        """The Authorizations cells of the day's summary, one per account."""
        lines = render_reports((replay_stream(stream, CHALLENGE).find_report(Day(day)),)).split("\n")
    ...
    assert list_cells(1) == ["Auth-A approved, hold 50.00; Auth-B declined, 90.00", "none"]
    assert list_cells(2) == ["Auth-A settled for 45.00; Auth-B declined, 90.00", "none"]
...
def test_capitalization_names_the_days_it_accrued() -> None:
    ...
    brief = render_reports((replay_stream(build_brief_stream(), CHALLENGE).find_report(Day(6)),)).split("\n")
    gaps = (
        make_credit("E1", 1, "1000.00"),
        make_debit("E2", 3, "1000.00"),
        make_credit("E3", 4, "1000.00"),
        make_debit("E4", 5, "1000.00"),
        make_credit("E5", 6, "10.000", account="ACC-002"),
    )
    gapped_lines = render_reports((replay_stream(gaps, CHALLENGE).find_report(Day(6)),)).split("\n")
    ...
    assert list_details(gapped_lines) == ["AED 1.20, accrued Days 1, 2, and 4", "BHD 0.004, accrued Day 6"]
...
def test_the_texts_beyond_output_target_follow_its_patterns() -> None:
    ...
    stream = (
        ...
        make_settlement("E3", 1, "Auth-A", "100.00"),
        make_settlement("E4", 1, "Auth-A", "40.00"),
        make_reversal("E5", 2, "E98"),
        make_reversal("E6", 2, "E99"),
        make_reversal("E7", 2, "INT-001-D1@D1"),
    )
    lines = render_reports(replay_stream(stream, CHALLENGE).reports[1:3]).split("\n")
    ...
    assert list_details(lines[:day_2]) == [
        "AED 400.00",
...
def test_a_partially_settled_authorization_prints_its_remaining_hold() -> None:
    ...
    lines = render_reports(replay_stream(stream, CHALLENGE).reports[2:4]).split("\n")
    ...
    assert find_cell(lines[:day_3], "| Authorizations", 1) == "Auth-A partially settled for 120.00, hold 80.00"
    assert find_cell(lines[day_3:], "| E4 ", 4) == "Auth-A settles for AED 40.00"
...
def test_a_step_that_fires_nothing_prints_its_note() -> None:
    ...
    brief = render_reports(replay_stream(build_brief_stream(), CHALLENGE).reports)
    aed_only_text = render_reports((replay_stream((make_credit("E1", 1, "100.00"),), CHALLENGE).find_report(Day(6)),))

    notes = {
        line.split(" | ")[4].strip()
        for text in (brief, aed_only_text)
...
def test_amb_014_a_rejected_event_prints_its_refusal(
    ...
    lines = render_reports((replay_stream(stream, CHALLENGE).find_report(Day(1)),)).split("\n")
