...
def test_amb_029_a_settlement_against_a_declined_authorization_is_force_posted() -> None:
    ...
    assert list_states(log, "Auth-A") == [Declined(Amount(make_aed("50.00")))]
    assert compute_closing(log, ACC_001, Day(3)) == make_aed("10.00")
...
def test_amb_029_a_settlement_after_a_final_one_is_force_posted() -> None:
    ...
    log = replay_stream(stream, CHALLENGE).find_log(Day(4))
    ...
    assert compute_closing(log, ACC_001, Day(4)) == make_aed("40.00")
...
def test_amb_030_a_settlement_above_its_hold_debits_in_full() -> None:
    ...
    assert sum_holds(log, ACC_001, Day(2)) == make_aed("0.00")
    assert list_fee_markers(log) == ["FEE-001-D2@D2"]
    assert compute_closing(log, ACC_001, Day(2)) == make_aed("-45.00")
