...
def test_amb_011_a_day_still_negative_is_not_charged_again() -> None:
    ...
    log = unwrap_ok(replay_stream((make_debit("E1", 1, "10.00"),), CHALLENGE)).find_log(Day(2))
...
def test_amb_027_a_bhd_account_is_charged_bhd_2_560() -> None:
    ...
    log = unwrap_ok(replay_stream((make_debit("E1", 1, "1.000", account="ACC-002"),), CHALLENGE)).find_log(Day(1))
...
def test_amb_035_a_reversed_refund_puts_its_fee_back_in_force() -> None:
    ...
    log = unwrap_ok(replay_stream(stream, week)).find_log(Day(7))
