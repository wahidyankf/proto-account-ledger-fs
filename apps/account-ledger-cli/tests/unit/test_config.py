...
def test_ledger_config_refuses_an_inverted_window() -> None:
    ...
    assert LedgerConfig.make((ACC_001,), Day(6), Day(1), frozenset()) == ConfigFault(
        "the first day 6 is after the last 1"
    )
    assert LedgerConfig.make((ACC_001, ACC_001), Day(1), Day(6), frozenset()) == ConfigFault(
        "ACC-001 is configured twice"
    )
    assert LedgerConfig.make((ACC_001,), Day(1), Day(6), frozenset({Day(7)})) == ConfigFault(
    ...
    assert CHALLENGE.accounts == (ACC_001, Account(AccountId("ACC-002"), make_bhd("0.000")))
