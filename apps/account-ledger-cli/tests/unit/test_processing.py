...
def test_amb_034_a_repeated_event_is_logged_as_a_duplicate_with_no_effect() -> None:
    ...
    result = replay_stream((e1, e1), CHALLENGE)
    log = result.find_log(Day(1))

    assert list_entries(log, "E1") == [Accepted(e1, Day(1)), Duplicate(e1, Day(1))]
    assert compute_closing(log, ACC_001, Day(1)) == make_aed("100.00")
...
def test_amb_034_a_repeated_reversal_or_settlement_is_a_duplicate(kind: str) -> None:
    ...
    result = replay_stream((*opening, repeated_event, repeated_event), CHALLENGE)
    single_log = replay_stream((*opening, repeated_event), CHALLENGE).find_log(Day(1))
    log = result.find_log(Day(1))

    assert list_entries(log, "E4")[1:] == [Duplicate(repeated_event, Day(1))]
    assert (compute_closing(log, ACC_001, Day(1)), sum_holds(log, ACC_001, Day(1))) == (
        compute_closing(single_log, ACC_001, Day(1)),
        sum_holds(single_log, ACC_001, Day(1)),
    )
    assert dict(result.find_report(Day(1)).errors) == {ACC_001.id: (), ACC_002.id: ()}
...
def test_amb_034_the_same_event_booked_another_day_is_refused() -> None:
    ...
    log = replay_stream((first_credit, retried_credit), CHALLENGE).find_log(Day(2))
...
def test_amb_034_a_reused_id_with_different_content_is_refused() -> None:
    ...
    log = replay_stream((first_credit, reused_credit), CHALLENGE).find_log(Day(1))
