...
recorded in [REJECTED](REJECTED.md), and each criterion below states its verdict and names the unit test in
`apps/account-ledger-cli/tests/unit/` that proves it. No figure below rests on a criterion alone.
...
- **C1.** The Day 2 closing ledger balance, evaluated at end of Day 5 and before any fee is assessed, is AED −370.00.
  ...
  same before or after the fees. _Test:_ `test_c1_day_2_closes_at_minus_370_at_end_of_day_5_before_fees`.
- **C2.** E7 causes exactly one overdraft fee to be assessed, on Day 2. _Ambiguities: AMB-002, AMB-003, AMB-016._
  **Refused**, in [REJECTED](REJECTED.md): E7 causes three fees, for Days 2, 4, and 5, all value-dated Day 5. _Test:_
...
- **C3.** The Day 4 settlement of Auth-A must be accepted. _Ambiguities: AMB-013._ **Accepted:** Auth-A is approved on
  ...
  (AMB-013); E7, arriving on Day 5, restates Day 2 but never reopens a decision already in the log (AMB-024). _Test:_
...
- **C4.** Any settlement referencing an authorization ID not present in the ledger must be rejected and the funds must
  ...
  force-post. _Test:_ `test_c4_e6_is_force_posted_for_180`.
- **C5.** If Auth-B is approved, its hold reduces available balance but not ledger balance. _Ambiguities: AMB-021._
  **Refused**, in [REJECTED](REJECTED.md). _Tests:_ `test_c5_auth_b_is_declined` and
...
- **C6.** After E9, all balances and fees return to their pre-E7 values. _Ambiguities: AMB-004, AMB-005._ **Refused**,
  ...
  0.79, and Day 6 closes at 285.76, not 285.79. _Tests:_ `test_c6_e9_restores_days_2_to_4_and_refunds_the_fees` and
...
- **C7.** The three BHD instalments in E10 must each be BHD 3.334. _Ambiguities: AMB-020._ **Refused**, in
  [REJECTED](REJECTED.md): 3 × 3.334 = 10.002; the ledger posts 3.333, 3.333, and 3.334. _Test:_
  `test_c7_e10_posts_3_333_3_333_3_334`.
- **C8.** If the rounded daily interest accruals do not sum to the capitalized total, the remainder is discarded.
  ...
  rounded interest events, so no remainder exists. _Test:_ `test_c8_capitalization_equals_the_sum_of_interest_events`.
