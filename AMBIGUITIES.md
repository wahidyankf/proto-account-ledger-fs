...
Entries are numbered in the order they first bear on [MOVEMENT](MOVEMENT.md)'s days, from Day 0 onwards, then on its
other sections; entries MOVEMENT never cites come last. Each entry is **Resolved** (the choice is recorded with its
reason) or **Open** (options and a recommendation are listed, and nothing is built on it yet). Figures come from a
scratch replay of the event stream, not from the ledger code, which does not exist yet; every figure is re-derived by
the test suite once it does.
...
Figures are ACC-001, in AED, with interest on the hindsight basis (AMB-005).
...
are new records, so nothing is mutated. Each reversal is booked on Day 6 and value-dated on the day of the fee it
reverses, so the fee and its reversal cancel in every closing they touch; the figures above assume that dating.
...
columns follow the two bases in AMB-005.
...
assessed", which only makes sense if assessment is a day-close step. The choice does not change Auth-B (AMB-021):
...
## AMB-020 — "Three equal instalments" of BHD 10.000
...
**Where.** Fees, fee reversals, interest, and capitalization all happen "at end of day", in no stated order.

**Status: Open.** Recommendation: fee re-evaluation, which assesses new fees and reverses any whose day is no longer
negative (AMB-004), then the day's interest accrual on the resulting closing, then — on Day 6 only — capitalization as
one credit value-dated Day 6. Day 6's accrual is computed before the capitalization credit, so interest never accrues on
itself, and the printed Day 6 closing includes the credit.
