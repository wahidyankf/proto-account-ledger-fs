...
A repository that supports Gherkin acceptance criteria additionally exposes exactly one `gherkin-implementation-review`.
One, not several: the review's value is that every scenario is walked, and two overlapping reviews each assume the other
covered the gap.
