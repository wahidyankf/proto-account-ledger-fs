...
1. **Placement and failure handling.** Hexagonal Architecture and
...
2. **Clarity and cost.** Code Clarity, Code as Liability, and Dependency Selection, with Shell Scripts for any script in
   scope.
...
4. **Test design.** Each test sits at its layer, per
   ...
   doubles, data, and any git fixture following Test Doubles, Test Data Isolation, and Git Fixture Isolation.
5. **Test-first evidence.** New or changed behaviour has a test, and the red, green, and refactor records that
   ...
   final state, never the order, per Software Quality Enforcement.
...
Checker, what scenario bindings assert to Gherkin Implementation Reviewer, a pinned change under review to the review
disciplines such as PR Review Integrity Checker, and documentation to Docs Checker.
